...
- **Scope to one user by default.** The Langfuse project a `langfuse-observability` install points at is frequently shared org/team-wide (one instance, many engineers), not per-seat. `langfuse_queries.py` therefore scopes every query to `--user`, defaulting to the `LANGFUSE_USER_ID` env var — always export it alongside the three credentials (see Prerequisites). Only pass `--user ''` to intentionally audit the whole shared project across every user, and say so explicitly in the report if you do, since every finding's "your usage" framing stops being accurate. A trace whose tool-call `input` references a filesystem path, username, or hostname that doesn't match the current user/machine is a sign scoping was missed — stop and re-run with `--user` set rather than reporting on someone else's session as if it were the current user's.
...
- `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`, and `LANGFUSE_USER_ID` available in the environment (or a `.env` file the user points you to).
  - **If they're not set**, and this project uses the `langfuse-observability` Claude Code plugin, the credentials already exist on the machine — they're just not exported:
    - `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_BASE_URL`, and `LANGFUSE_USER_ID` are in `~/.claude/settings.json` under `pluginConfigs["langfuse-observability@langfuse-observability"].options`.
    - `LANGFUSE_SECRET_KEY` isn't stored in settings.json — it's under `pluginSecrets["langfuse-observability@langfuse-observability"].LANGFUSE_SECRET_KEY` in `~/.claude/.credentials.json` (or, on older installs, base64-encoded inside `env.OTEL_EXPORTER_OTLP_HEADERS` as an `Authorization: Basic <base64(pk:sk)>` header — decode and take the `sk` half).
    - Export all four before running the queries below; don't hardcode them into a file. `LANGFUSE_USER_ID` is what scopes the audit to this user's own traces (see the "Scope to one user by default" constraint above) — without it every query below silently runs against the whole shared project.
...
1. `trace_outliers` — every **trace** (not session) that exceeds 3x the window's median (the `--factor` default) on at least one of: cost, tool-call count, input tokens, output tokens, **`context_depth_per_call`**. Each entry lists which metric(s) tripped and by what ratio. This is trace-first rather than session-first on purpose: a session aggregates many turns, so ranking by session cost buries a single runaway trace inside an otherwise-cheap session, and misses a trace that's a tool-call/token outlier without yet being a cost outlier.
   ...
   - Each trace also carries `generation_count` (number of LLM calls, i.e. agentic turns, in that trace). A trace with `generation_count` in the tens and a `context_depth_per_call`/`depth_ratio` that's large and roughly flat across those calls, with `duplicate_tool_calls` for that trace low/absent, is cost from turn count × baseline size rather than any single wasteful call — see the "Context-multiplication" pattern in Step 1c.
...
4. `underuse_profile.findings` is empty, **and**
5. `session_sequences` is empty.
...
2. Findings/traces flagged on 2+ metrics in `outlier_reasons`/`trace_outliers` (e.g. both tool-call count and output tokens) — the least ambiguous offenders on the loop/pathology side, and sort first among ties
3. Findings where duplicate-call count / trace **cost-bearing** observation count > 15%, ranked by `wasted_output_bytes` rather than `repeat_count` — one repeated 500KB read costs far more than fifty repeated `git status` calls (this is exactly `ranked_findings`'s sort order; cross-check against `duplicate_tool_calls.json` if the raw share matters for the report)
4. Remaining findings/traces, ranked by `outlier_max_ratio`/highest ratio-to-median on any single metric, with `excess_cost` as the tiebreak
...
6. `cross_trace_duplicates` has a signature spanning 3+ traces, or its `trace_count` is a large share of the session's `trace_count` — a later trace paid to redo what an earlier trace in the same session had already done
7. `context_growth` shows an input-token ratio ≥3x with a tool-call ratio near 1 (i.e. cost grew from accumulated/repeated context, not from new work) — especially when it recurs across several trace-pairs in the session
8. `escalation_after_failure` appears at all — a failed trace immediately followed by a pricier-model retry in the same session is worth root-causing even at low volume, since it implies a fixable root cause was never addressed
