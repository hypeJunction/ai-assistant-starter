...
│   ├── .instructions.md    # Global execution protocol (this file)
...
│   ├── domains/            # Override/extend domains
...
│
└── [PROVIDER].md           # Provider-specific entry point (e.g., CLAUDE.md)
```
...
- Looking for `domains/typescript.instructions.md`:
  ...
  2. If not found, use `.ai-assistant/domains/language/typescript.instructions.md`
...
| Command | Purpose |
|---------|---------|
| `/implement` | Full workflow: explore → plan → code |
...
**Section order:** TASK → STATUS → WARNING → ERROR → ACTION → NEXT
...
**Full reference:** [domains/communication.instructions.md](./domains/_universal/communication.instructions.md)
...
**Required for:** E2E tests, Storybook interaction tests, DOM-based unit tests

Browser-based test plans must use element locators and action verbs compatible with `/agent-browser`:
...
|---------|--------|---------|
| Role + Name | `button "Submit"` | `I click button "Submit"` |
| Label | `input[label="Email"]` | `I type "text" into input[label="Email"]` |
| Placeholder | `input[placeholder="Search"]` | `I type "query" into input[placeholder="Search"]` |
| Text content | `text "Welcome"` | `I see text "Welcome"` |
| Test ID | `[data-testid="submit-btn"]` | `I click [data-testid="submit-btn"]` |

**Action Verbs:**
| Action | Usage |
|--------|-------|
| `navigate to` | `I navigate to "/path"` |
| `click` | `I click button "Name"` |
| `type ... into` | `I type "value" into input[label="Field"]` |
| `clear` | `I clear input[label="Field"]` |
| `select ... from` | `I select "Option" from combobox "Country"` |
| `check/uncheck` | `I check checkbox "Terms"` |
| `hover` | `I hover over button "Menu"` |
| `press` | `I press "Enter"` |
| `upload` | `I upload "file.pdf" to input[label="Document"]` |
| `drag ... to` | `I drag [data-testid="item"] to [data-testid="dropzone"]` |

**Assertions:**
| Assertion | Usage |
|-----------|-------|
| `I see` | `I see text "Success"`, `I see heading "Title"` |
| `I don't see` | `I don't see text "Error"` |
| `is visible/hidden` | `button "Submit" is visible` |
| `is enabled/disabled` | `button "Submit" is disabled` |
| `contains` | `input[label="Name"] contains "John"` |
| `URL contains` | `URL contains "/dashboard"` |
| `count` | `I see 3 listitem` |

See [domains/testing/vitest.instructions.md](./domains/testing/vitest.instructions.md) for implementation examples.

## Domain-Specific Guidelines

Domains are organized by category in `.ai-assistant/domains/`. During `/init`, relevant domains are copied to `.ai-project/domains/` based on your stack.

**Universal domains** (always included):
- **[Git](./domains/_universal/git.instructions.md)** - Branch naming, commit messages
- **[Security](./domains/_universal/security.instructions.md)** - Security practices
- **[Communication](./domains/_universal/communication.instructions.md)** - User interaction patterns

**Stack-specific domains** (copied based on detection):
- **[TypeScript](./domains/language/typescript.instructions.md)** - Type usage, imports, exports
- **[Testing (Vitest)](./domains/testing/vitest.instructions.md)** - Test patterns, mocking
- **[API (REST)](./domains/api/typescript-rest.instructions.md)** - REST API patterns

See [INDEX.md](./INDEX.md) for the full list of available domains.
...
|----------|---------|--------------|
| 🔍 [Explorer](./chatmodes/explorer.chatmode.md) | Understand code | Read-only exploration, pattern discovery |
| 📋 [Planner](./chatmodes/planner.chatmode.md) | Design approach | Create plans, identify risks, read-only |
| 👨‍💻 [Developer](./chatmodes/developer.chatmode.md) | Code implementation | Edit files, run commands, create tests |
| 🧪 [Tester](./chatmodes/tester.chatmode.md) | Quality assurance | Create tests, verify behavior |
| 👁️ [Reviewer](./chatmodes/reviewer.chatmode.md) | Code review | Read-only, suggest improvements |
| 🏗️ [Architect](./chatmodes/architect.chatmode.md) | Design and planning | Read-only, create ADRs |
| 🐛 [Debugger](./chatmodes/debugger.chatmode.md) | Diagnose problems | Read + limited bash for investigation |
| 💾 [Committer](./chatmodes/committer.chatmode.md) | Stage and commit | Git operations with user confirmation |
| ♻️ [Refactorer](./chatmodes/refactorer.chatmode.md) | Multi-file changes | Full access with file list tracking |
| 🚀 [Releaser](./chatmodes/releaser.chatmode.md) | Create PRs | Git/gh commands, branch management |
