...
- `trash` workflow skill — abandon a session instead of shipping it: record the reason, optionally offer to `git stash` uncommitted work (with explicit per-run approval, never dropped automatically), and reuse `done`'s session-state/Langfuse plumbing to score the outcome and report cost. Bookkeeping only — never touches git/GitHub beyond the opt-in stash
- `pivot` workflow skill — explicitly pause the current thread and re-enter Plan Mode for a new direction (acknowledge the pause, determine the new direction inline or by asking, then `EnterPlanMode`). Manual counterpart to a new `prompt-context-router` axis: a companion `PostToolUse` hook (`post-tool-hook.js`) watches for `ExitPlanMode` followed by a mutating tool call (`Edit`/`Write`/`NotebookEdit`/`Bash`) and records `implementedSincePlan` into the same tmpdir state file `hook.js` already uses for its `extremeNudged`/`architecturePivotNudged` flags; `hook.js` then denies a pivot-shaped prompt once per plan cycle (`PROMPT_CONTEXT_ROUTER_POST_PLAN_PIVOT_MODE`, defaults to `enforce` — the one axis in this hook that defaults to blocking rather than advisory-only) until `EnterPlanMode` is called again, downgrading to advisory after the first deny and resetting on the next `ExitPlanMode`. `apply-template`'s Step 5.8 installs both hooks together; `loadState`/`saveState` extended to carry the two new fields through without dropping the existing ones
- `session_log.py` (shared by `done` and `trash`) gains a numeric `session_outcome` Langfuse score (9 for `/done`, 1 for `/trash`, alongside the existing `session_done` boolean) and a per-session cost report — total USD + input/output/cache-read/cache-write token breakdown, computed by parsing the session's own local JSONL transcript (`~/.claude/projects/<slug>/<id>.jsonl`, deduplicated on `message.id` the same way `session-retro`'s transcript analyzer does, priced via a `RATES` table mirroring `cost-audit`'s) rather than querying Langfuse — no network call or credentials needed for the cost figure itself, only for the score. Printed after every `done`/`trash` run; both skills fail soft to "no local transcript found" instead of erroring when the transcript is missing, and to "no Langfuse credentials configured" for the score post specifically. A new opt-in `SESSION_COST_RETRO_THRESHOLD_USD` env var makes the script print an extra advisory line when a session's cost exceeds it, which both skills' `SKILL.md` now instruct the assistant to follow up on by offering `/session-retro` via `AskUserQuestion`. `cmd_done` no longer exits with an error when no prior `/start` session record exists — it backfills one retroactively (`backfilled: true`) instead
